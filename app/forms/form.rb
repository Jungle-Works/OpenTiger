module Form
  NewMarketplace = FormUtils.define_form("NewMarketplaceForm", :admin_email, :admin_first_name, :admin_last_name, :admin_password, :admin_phone,:admin_phone_code,
    :marketplace_country, :marketplace_language, :marketplace_name,:access_token, :marketplace_type, :rental_vertical
  ).with_validations do
    validates_presence_of :admin_email, :admin_first_name, :admin_password, :admin_phone
    validates_format_of   :admin_email, with: /\A[A-Z0-9._%\-\+\~\/]+@([A-Z0-9-]+\.)+[A-Z]+\z/i
    validates_length_of   :admin_password, minimum: 8
    validates_length_of   :admin_phone, minimum: 8
