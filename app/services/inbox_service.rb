module InboxService
  ...
  def inbox_data_count(person_id, community_id, inbox_view_type)
    query_inbox_data_count(person_id, community_id, inbox_view_type)
  end

  def inbox_data(person_id, community_id, limit, offset, inbox_view_type)
    query_inbox_data(person_id, community_id, limit, offset, inbox_view_type)
  end
  ...
  def query_inbox_data(person_id, community_id, limit, offset, inbox_view_type)
    conversation_ids = get_conservation_ids_by_inbox_view_type(person_id, inbox_view_type)
  ...
  def query_inbox_data_count(person_id, community_id, inbox_view_type)
    conversation_ids = get_conservation_ids_by_inbox_view_type(person_id, inbox_view_type)
    return 0 if conversation_ids.empty?
  ...
  def get_conservation_ids_by_inbox_view_type(person_id, inbox_view_type)
    if inbox_view_type == "enquiry"
      conversation_ids = Participation.where(person_id: person_id).direct_participants.pluck(:conversation_id)
    elsif inbox_view_type == "bookings"
      conversation_ids = Participation.where(person_id: person_id).starter_without_direct_participants.pluck(:conversation_id)
    elsif inbox_view_type == "hostings"
      conversation_ids = Participation.where(person_id: person_id).recipient_without_direct_participants.pluck(:conversation_id)
    else
      conversation_ids = Participation.where(person_id: person_id).pluck(:conversation_id)
    end
