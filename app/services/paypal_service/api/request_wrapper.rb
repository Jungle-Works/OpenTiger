module PaypalService::API::RequestWrapper
  ...
  def with_success(cid, txid, request, opts = { error_policy: {} }, &block)
    ...
    if(request[:method] == :do_capture && response[:error_code] == "10602")
      response = paypal_merchant.do_request({:method=>:get_transaction_details, :receiver_username=>request[:receiver_username], :transaction_id=> request[:authorization_id]})
    end
    
    if(request[:method] == :get_transaction_details)
      response = response.without(:payment_id,:payment_total,:fee_total,:payment_date)
    end
