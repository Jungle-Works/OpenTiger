...
module TransactionService::Process
  ...
  class Preauthorize
    ...
    def create(tx:, gateway_fields:, gateway_adapter:, force_sync:)
      ...
      puts " sakshi 21 ,,,, insided create .... force sync is :" , force_sync
    ...
    def do_create(tx, gateway_fields)
      ...
      puts " the completion object is : " , completion

      if completion[:success] && completion[:sync]
        puts "inside completion success"
      ...
      puts "not in completion success..."
      completion[:response]
    end
    ...
    def finalize_create(tx:, gateway_adapter:, force_sync:)
      ...
      if !force_sync
        ...
        puts "inside do finalize create...sakshi 23"
    ...
    def do_finalize_create(transaction_id, community_id)
      ...
      puts "sakshi 24 , the tx is : " , tx

      gateway_adapter = TransactionService::Transaction.gateway_adapter(tx.payment_gateway)

      puts "the gateway adapter is sakshi 25 .... gateway adapter..." , gateway_adapter

      res =
        if tx.current_state == :preauthorized
          puts " the tx current state is : ...  preauthorize sakshi 26" 
          Result::Success.new()
        else
          puts " the transaction current state is some other .... "
          booking_res =
