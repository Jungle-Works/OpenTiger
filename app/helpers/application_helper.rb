...
module ApplicationHelper
  ...
  def initialize_i18n(community)
    m_community = Maybe(community)
    ...
    I18nHelper.initialize_community_backend!(community_id, community_locales)  
  end
  ...
  def service_name
    if @current_community
      ...
      APP_CONFIG.global_service_name || "Yelo"
    end
  end
  ...
  def self.store_community_service_name_to_thread_from_community(community=nil)
    ser_name = APP_CONFIG.global_service_name || "Yelo"
  ...
  def self.fetch_community_service_name_from_thread
    Thread.current[:current_community_service_name] || APP_CONFIG.global_service_name || "Yelo"
  end
  ...
  def admin_links_for(community)
    links = [
      ...
      },
      {
        ...
        :path => "https://jungleworks.com/yelo/rentals-guide/",
        :name => "academy"
      }
    ]
    ...
    links += [
      ...
      :path => admin_manage_landing_page_index_path,
      :name => "landing_page"
      }
    ]
    ...
    links += [
      ...
      :topic => :configure,
      :text => "Themes",
      :icon_class => icon_class("furniture"),
    ...
    links += [
      ...
      {
        :topic => :configure,
        :text => t("admin.checkout_fields.index.checkout_fields"),
        :icon_class => icon_class("order_types"),
        :path => admin_checkout_settings_path,
    ...
    links << {
      ...
      :icon_class => icon_class("icon_tasks"),
      :path => admin_listing_shapes_path,
      :name => "listing_shapes"
    }

    if PaypalHelper.paypal_active?(@current_community.id) || StripeHelper.stripe_provisioned?(@current_community.id) || RazorpayHelper.razorpay_provisioned?(@current_community.id)
      links << {
        :topic => :configure,
    ...
    links << {
      :topic => :configure,
      :text => t("admin.communities.content_pages.content_pages"),
      :icon_class => icon_class("terms"),
      :path => admin_content_page_path(@current_community),
      :name => "content_pages"
    }
  ...
  def settings_links_for(person, community=nil, restrict_for_admin=false)
    ...
    if !restrict_for_admin && (paypal_ready || stripe_ready || razorpay_ready )
      links << {
  ...
  def show_cover_image
    return false if params[:controller] == "content_page_load"
