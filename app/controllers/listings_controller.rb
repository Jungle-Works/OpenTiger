class ListingsController < ApplicationController
  ...
  def show
    ...
    render action: "#{theme_file_path}show"
  end
  ...
  def new
    ...
    render locals:{theme_class: get_theme_classes}
  end
  ...
  def create
    ...
    service = Admin::ListingsService.new(community: @current_community, params: params)
    ...
    ActiveRecord::Base.transaction do
      ...
      if @listing.save
        blocked_dates = params[:listing][:blocked_dates] ? JSON.parse(params[:listing][:blocked_dates]) : []
        blocked_dates.each {|a| "#{@listing.listing_blockdates.create(:date => a)}"} if blocked_dates.present?
  ...
  def edit
    ...
    form_sidebar_links.each do |links|
      if links[:kind].include? "price" 
        selected_tabs << links if shape[:price_enabled]
      elsif links[:kind].include? "custom"
        selected_tabs << links unless @custom_field_questions.length == 0
      else
        selected_tabs << links
      end
    end

    render locals: { form_content: form_locals(shape), sidebar_links: selected_tabs, theme_class: get_theme_classes,form_content_partial_path: get_partial_path }
  end
  ...
  def update
    ...
    if update_successful
      if params.dig(:listing, :blocked_dates).present? 
        dates = JSON.parse(params.dig(:listing, :blocked_dates)) 
        already_included_dates = @listing.listing_blockdates.where(:date => dates).map{|a| a.date.strftime("%Y-%m-%d")} 
        extra_dates = @listing.listing_blockdates.where.not(:date => dates).destroy_all 
        dates_to_be_added = dates - already_included_dates 
        dates_to_be_added.each {|a| "#{@listing.listing_blockdates.create(:date => a)}"} 
      end
  ...
  def form_content
   ...
   form_sidebar_links.each do |links|
    ...
    if allow_posting
      render :partial => get_partial_path, locals: form_locals(shape).merge(run_js_immediately: true, 
        sidebar_links: selected_tabs, add_form: true)
    else
  ...
  def fetch_block_dates
    @listing_block_dates = (@listing.listing_blockdates.map{|a| a.date.strftime("%Y-%m-%d")} + @listing.fetch_all_bookings_dates.map{|b| b.strftime("%Y-%m-%d")}).uniq
    # @listing_block_dates = @listing.listing_blockdates.map{|a| a.date.strftime("%Y-%m-%d")}
    puts "kunal12#{@listing_block_dates}"
  end
  ...
  def form_sidebar_links
    links = [
      # {text: "Availability", kind: "availability"},
      # {text: "Categories", kind: "categories"},
      {text: t("listings.new.description"), kind: "description"},
      {text: t("listings.new.pricing"), kind: "price"},
      {text: t("listings.new.location"), kind: "location"},
      {text: t("listings.new.additional_info"), kind: "custom"},
      {text: t("listings.new.photos"), kind: "photos"},
    ]
  end
  ...
  def get_partial_path
    case @current_theme_identifier
    when "flex-theme"
      partial_path = "listings/flex-theme/form_content"
    else
      partial_path = "listings/form/form_content"
    end
