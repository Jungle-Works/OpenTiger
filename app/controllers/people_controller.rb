class PeopleController < Devise::RegistrationsController
  ...
  def new
    @selected_tribe_navi_tab = "members"
    redirect_to homepage_without_locale_path(locale: nil) and return if logged_in?
    ...
    render action: get_partial_path, locals: {image_s3_options: image_s3_options}
  end
  ...
  def create
    ...
    email = nil
    uri = URI('https://prod-api.bulbul.io/add_deal') 
    ...
    contact_person = { 
    :name => params[:person][:given_name] + params[:person][:family_name], 
    :email => [ { 
    :email => params[:person][:email] 

    } ], 
    :phone => [ { 
    :phone => params[:person][:phone_number] 
    }] 

    } 
    if @current_community[:id] == 63 

      req.body = { 
        :name => params[:person][:given_name] + params[:person][:family_name], 
        :user_id => "63kv1wfr09snqj5g63po", 
        :assign_to => "63kv1wfr09snqj5g63po", 
        :deal_stage => 1, 
        :contact_person => contact_person, 
        :workflow => '3949fbe6-279b-4cb8-9ae2-1a4637826a7d' 
      ...
      req.body = { 
        :name => params[:person][:given_name] + params[:person][:family_name], 
        :user_id => "i4fcf0oxs4dxlbji37ei", 
        :assign_to => "i4fcf0oxs4dxlbji37ei", 
        :deal_stage => 1, 
        :contact_person => contact_person, 
        :workflow => '0abb38d0-5f39-40ee-bea9-3a6a0f769050' 
    ...
    params[:person][:deal_id] = parsed_json["data"]["deal_id"]
    ...
    begin
      ...
      #admin = Person.find_by(community_id: @current_community[:id],is_admin: 1)

      #adminEmail = Email.find_by(person_id: admin.id)
      
      #admin_email = adminEmail[:address]
  
  
      #if UserMailer::Mandrill.new_user_added(admin_email,@current_community[:ident],params[:person][:email])
  ...
  def update
    ...
    begin
      ...
      if target_user.update_attributes(person_params.except(:email_attributes))
        puts " params areeeeee"  ,person_params.except(:email_attributes)
        ...
        puts " there was a error !!!! ", target_user.errors.full_messages 
        puts " the params created are : " , person_params.except(:email_attributes)
