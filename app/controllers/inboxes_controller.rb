class InboxesController < ApplicationController
  ...
  def show
    ...
    count = InboxService.inbox_data_count(@current_user.id, @current_community.id, params[:type])
    ...
    if request.xhr?
      ...
      render "inboxes/#{theme_file_path}show", locals: {
        inbox_rows: paginated_inbox_rows,
        payments_in_use: @current_community.payments_in_use?,
        coversation_types: getConversationTypes
  ...
  def inbox_title(inbox_item, payment_sum)
    title = if InboxService.last_activity_type(inbox_item) == :message
      ...
      action_messages.present? ? action_messages.last[:content] : "YYYY"
    end
  end
  ...
  def getConversationTypes
    conversationTypes =[
      {"type"=> "All", "href"=> person_inbox_path + "/?type=all-conversations"  },
      {"type"=> "Hostings", "href"=> person_inbox_path + "/?type=hostings"},
      {"type"=> "Bookings", "href"=> person_inbox_path + "/?type=bookings"},
      {"type"=> "Enquiry", "href"=> person_inbox_path + "/?type=enquiry"},
    ]
  
    
  end

  def highlight_conversation_type(params)
