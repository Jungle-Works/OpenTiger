class Admin::PaymentPreferencesController < Admin::AdminBaseController
  ...
  def index
    ...
    puts " the more locals prev..." , more_locals

    if @paypal_enabled
      more_locals.merge!(paypal_index)
    end

    puts " the more locals after paypal .. " , more_locals

    if @stripe_enabled
      more_locals.merge!(stripe_index)
    end

    puts " the more after stripe enabled...."

    if @razorpay_enabled  
      more_locals.merge!(razorpay_index)
    end   

    puts " the more locals after razorpay.... " , more_locals
    ...
    razorpay_connected = view_locals[:razorpay_enabled] && view_locals[:razorpay_account].present? && view_locals[:razorpay_account][:api_verified]
    ...
    payment_locals = {
      ...
      payments_connected: stripe_connected || paypal_connected || razorpay_connected,
      ...
      razorpay_allowed: TransactionService::AvailableCurrencies.razorpay_allows_country_and_currency?(@current_community.country, @current_community.currency),
      ...
      razorpay_ready: RazorpayHelper.community_ready_for_payments?(@current_community.id),
      paypal_enabled_by_admin: !!paypal_tx_settings[:active],
      stripe_enabled_by_admin: !!stripe_tx_settings[:active],
      razorpay_enabled_by_admin: !!razorpay_tx_settings[:active]
    }
  ...
  def ensure_payments_enabled
    ...
    @razorpay_enabled = RazorpayHelper.razorpay_provisioned?(@current_community.id)
    puts " the razorpay enabled is : "  , @razorpay_enabled
    unless @paypal_enabled || @stripe_enabled || @razorpay_enabled
  ...
  def razorpay_index  
    razorpay_account = razorpay_tx_settings 
    {
      razorpay_account: razorpay_account,
      razorpay_api_form: RazorpayApiKeysForm.new
    }
  end   
  ...
  def razorpay_tx_settings
    Maybe(tx_settings_by_gateway(:razorpay))
    .select { |result| result[:success] }
    .map { |result| result[:data] }
    .or_else({})
  end   
  ...
  def build_prefs_form(params = nil)
    puts " inside build preferences form "
    currency = @current_community.currency
   puts " the currency is : " , currency
    data = {paypal_prefs_form: nil, stripe_prefs_form: nil, razorpay_prefs_form: nil}
   puts " the data is " , data
    ...
    puts " after paypal check ... " , data
    if @stripe_enabled
      data[:stripe_prefs_form] = prefs_form_from_settings(stripe_tx_settings, 0, currency)
    end

    if @razorpay_enabled   
      data[:razorpay_prefs_form] = prefs_form_from_settings(razorpay_tx_settings,0,currency)
    end   

    puts " after stripe enable check ... " , data
  ...
  def update_payment_preferences
    ...
    if form.valid?
      ActiveRecord::Base.transaction do
        ...
        if razorpay_tx_settings.present? && (params[:gateway] == 'razorpay'|| form.mode == 'general')
          tx_settings_api.update(base_params.merge(payment_gateway: :razorpay))
        end
      end
  ...
  RazorpayApiKeysForm = FormUtils.define_form("RazorpayApiKeysForm",
   :key_id,
   :key_secret).with_validations do   
    validates_format_of :key_id, with: Regexp.new(APP_CONFIG.razorpay_key_id_pattern)
    validates_format_of :key_secret, with: Regexp.new(APP_CONFIG.razorpay_secret_key_pattern)
   end

  def process_update_razorpay_keys  
    api_form = RazorpayApiKeysForm.new(params[:razorpay_api_keys_form])
     if api_form.valid? && api_form.key_secret.present?  
      if !@razorpay_enabled
        tx_settings_api.provision({ community_id: @current_community.id,
          payment_process: :preauthorize,
          payment_gateway: :razorpay,
          api_private_key: api_form.key_secret,
          api_publishable_key: api_form.key_id
         })
        else   
      ...
      #check balance left 
      tx_settings_api.api_verified(community_id: @current_community.id, payment_gateway: :razorpay, payment_process: :preauthorize)
      tx_settings_api.activate(community_id: @current_community.id, payment_gateway: :razorpay, payment_process: :preauthorize)
      flash[:notice] = "Razopay keys verified ..... "
     end 
  end  
  ...
  def ensure_params_payment_gateway
    ['stripe', 'paypal', 'razorpay'].include?(params[:payment_gateway])
  end
end
