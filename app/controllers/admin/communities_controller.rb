class Admin::CommunitiesController < Admin::AdminBaseController
  ...
  def postings 
    puts "Posting  is Called !!! "  
    puts @current_community[:id]
    community = Community.find_by(id: @current_community[:id])
    if community[:IsListingApprovalRequired] == true
      community.update_attribute(:IsListingApprovalRequired, false)
    else  
      community.update_attribute(:IsListingApprovalRequired, true)
    end


    redirect_to  '/admin/communities/' +  String(@current_community[:id]) +'/listings'
    
  end
  def approveListing
    puts "Approve Listing Called" , params.as_json["id"]
    listing = Listing.find_by(id: params[:id])
    puts " listings is : " , listing
    # listing.update_attribute(:is_approved, true)
    redirect_to  '/admin/communities/' +  String(@current_community[:id]) +'/listings'

  end

  def dissaproveListing

    puts "Dissapprove Called" , params
    listing = Listing.find_by(id: params[:id])
    # listing.update_attribute(:is_approved, false)
    redirect_to  '/admin/communities/' +  String(@current_community[:id]) +'/listings'
  ...
  def toggleListingView

    puts "The Community ID is : "  , @current_community.id
  ...
  def edit_welcome_email
    ...
    render "edit_welcome_email", locals: {
             ...
             link_to_sharetribe: "https://jungleworks.com/yelo/marketplace-like-airbnb/=#{@current_community.ident}.yelo.red&utm_medium=referral&utm_campaign=nowl-admin-panel"
           }
  end
  ...
  def get_table_header
    puts "The Button Was clickedd.......nikhil sharma"
  end
