class Admin::CustomFieldsController < Admin::AdminBaseController
  ...
  def update_date
    date_params = params.require(:community).permit(:show_date_filter)
    success = @current_community.update_attributes(date_params)

    if success
      redirect_to admin_custom_fields_path
    else
      flash[:error] = "Date field editing failed"
      render :action => :edit_date
    end
  end
