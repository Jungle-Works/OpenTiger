class LandingPageController < ActionController::Metal
  ...
  def index
    ...
    footer = FooterPresenter.new(com, request.env[:current_plan])
    if com.footer_enabled?
      @custom_footer = setFooterData(footer)
    else
      @custom_footer = {"enabled" => false}
    end
    fetch_theme
    cid = com.id
  ...
  def setFooterData(footer)

    {"links"=> footer.links? ? footer.links.map{ |link| { "href" => {"value" => link["url"]}, 
    "label" => link["title"]}} : [],
    "enabled"=> true,
     "social"=> footer.social? ? footer.social.map{ |link| {"url" => link["url"], 
    "service" => link["provider"]}} : [], 
    "theme"=> footer.theme, "copyright"=> footer.copyright}
  ...
  def preview
    ...
    begin
      structure = CLP::LandingPageStore.load_structure(cid, 3)
  ...
  def fetch_custom_translation(cid,locale)
    community_translations = CommunityTranslation.where(:community_id => cid.id, :locale => locale)
  ...
  def render_landing_page(default_locale:, locale_param:, structure:, cta:)
    ...
    if user(request).present?
      current_user = {
        "given_name" => user(request)["given_name"],
        "is_admin" => user(request)["is_admin"]
      }
      get_auth_token
    else
      current_user = ""
    ...
    render_to_string :landing_page,
           locals: { font_path: FONT_PATH,
                     ...
                     auth_user: @auth_user,
                     hippo: {props_endpoint: ui_api_hippo_props_path}
                   }
  end
  ...
  def topbar_props(community, community_customization, request_path, locale_param, topbar_locale, landing_page)
    ...
    fetch_custom_translation(community,I18n.locale)
