...
class PreauthorizeTransactionsController < ApplicationController
  ...
  def initiate
    ...
    validation_result = params_validator.validate(params).and_then { |params_entity|
      ...
      if params[:razorpay_payment_id].present? && params[:razorpay_order_id]
        tx_params[:razorpay_payment_id] = params[:razorpay_payment_id]
        tx_params[:razorpay_order_id] = params[:razorpay_order_id]
      end  

      if params[:app_session_token].present?
        tx_params[:app_session_token] = params[:app_session_token]
      end
  ...
  def create_preauth_transaction(opts)
    ...
    TransactionService::Transaction.create({
      ...
      force_sync: opts[:payment_type] == :stripe || ( opts[:payment_type] == :razorpay  || opts[:force_sync] ) )
  end
  ...
  def price_break_down_locals(tx_params, listing)
    ...
    #additional_price = Money.new(((10*100)/quantity.to_f).round(2),@current_community.currency)
  ...
  def initiation_success(tx_params)
    ...
    render "listing_conversations/#{theme_file_path}initiate",
           locals: {
             start_on:   tx_params[:start_on],
             end_on:     tx_params[:end_on],
             start_time: tx_params[:start_time],
             ...
             form_action: (params[:app_session_token].present? ? initiated_order_path(person_id: @current_user.id, listing_id: listing.id, :app_session_token => params[:app_session_token]) : initiated_order_path(person_id: @current_user.id, listing_id: listing.id)),
             ...
             razorpay_in_use: RazorpayHelper.user_and_community_ready_for_payments?(listing.author_id, @current_community.id)
           }
  end
