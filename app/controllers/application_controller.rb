...
class ApplicationController < ActionController::Base
  ...
  def get_auth_token
    ...
    # uri = URI('https://#{APP_CONFIG.auth_api_key}/get_user_detail')
    uri = URI(@url)
    ...
    req.body = { 
      :field_names => "*",
      :user_id => @current_community[:user_id], 
      :auth_key => @authKey,
      :offering => 2
      }; 
      begin
        ...
        puts "the access token is : " , @authBody["data"][0]["access_token"]
        @auth_user[:access_token] =  @authBody["data"][0]["access_token"]
  ...
  # Before filter to get the current theme
  def fetch_theme
    if @current_community.present?
      current_community_themes = @current_community.community_themes
      if current_community_themes.present?
        current_enabled_theme = current_community_themes.where(enabled: true)
        @current_theme = current_enabled_theme.last
        unless current_enabled_theme.present?
          current_community_themes.last.update(enabled: true)
          @current_theme = current_community_themes.last
        end
      else
        theme = Theme.first
        @current_theme = @current_community.community_themes.create(theme_id: theme.id, content: theme.content, enabled: true, released_at: Time.now)
      end
      theme_content = JSON.parse(@current_theme.content)
      @current_theme_identifier = theme_content["theme_unique_identifier"]
      request.env[:theme_id] = Maybe(@current_theme.try(:id)).or_else(nil)
    else
      p "Current theme can't be found as community can't be fetched."
    end
  end
  ...
  def warn_about_missing_payment_info
    if @current_user
      ...
      razorpay_community  = RazorpayHelper.community_ready_for_payments?(@current_community.id)
      ...
      razorpay_ready     = RazorpayHelper.user_razorpay_active?(@current_user.id,@current_community.id)
      ...
      if razorpay_community && razorpay_ready
        accept_payments << :razorpay
      end
  ...
  def fetch_custom_translation
    community_translations = CommunityTranslation.where(:community_id => @current_community.id, :locale => I18n.locale)
    hsh = HashWithIndifferentAccess.new
    community_translations.each{ |trn| hsh[trn.translation_key] = trn.translation }
    @dynamic_translations = hsh
  end
  ...
  def check_auth_token
    ...
    if person
      ...
      #path_without_auth_token = path_without_auth_token + '/admin/getting_started_guide'
      puts " the page_without auth token :  " ,  path_without_auth_token

      CommunityMembership.find_by(person_id: @current_user.id).update_attribute(:status , 'accepted')
    
      if params[:app_session_token].present?
        if TbActiveAppSession.find_by_session_token(params[:app_session_token]).try(:person) != @current_user && @current_user.is_admin.present?
          redirect_to admin_url
        else
          puts "webview url is: #{request.protocol}#{@current_user.community.domain}#{path_without_auth_token}"
          redirect_to path_without_auth_token
        end
      else
        if @current_user.is_admin.present?
          redirect_to admin_url
        else
          redirect_to path_without_auth_token
        end
  ...
  def display_onboarding_topbar?
    ...
    return false if params[:app_session_token].present?
