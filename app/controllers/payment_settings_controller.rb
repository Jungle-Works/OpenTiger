class PaymentSettingsController < ApplicationController
  ...
  def add_razorpay_account
    @res = razorpay_account_api.create(params: razorpay_account_params, community: @current_community.id, person: @current_user.id)
    if @res.success == true  
      status = 200 
    else 
      status = 400  
    ...
    @response = {
      "status" =>  status,
      "message" => @res.success == true ? "Account Added !! . You may need to verify your account details" : @res.error_msg 
    }
  ...
  def razorpay_account_params
    params.permit(:name, :email, :business_name, :business_type, :account_number, :ifsc_code, :account_type, :beneficiary_name, :accept_rp_tnc)
  end
  
  def razorpay_account_api 
