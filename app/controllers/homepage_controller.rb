...
class HomepageController < ApplicationController
  ...
  def index
    ...
    if FeatureFlagHelper.feature_enabled?(:searchpage_v1)
      ...
      show_date_filter = @current_community.show_date_filter
    ...
    listing_count = APP_CONFIG.map_listings_limit 
    ...
    map_search_result = find_listings(params: params,
                                  current_page: current_page,
                                  listings_per_page: listing_count,
                                  filter_params: compact_filter_params,
                                  includes: includes.to_set,
                                  location_search_in_use: location_in_use,
                                  keyword_search_in_use: keyword_in_use,
    ...
    if @view_type == 'map'|| @view_type == 'grid' || @view_type == 'list'
    ...
    if FeatureFlagHelper.feature_enabled?(:searchpage_v1)
      ...
      search_result.on_success { |listings|
        ...
        addRatingToListingsArray
        
        if @view_type == "grid" then
          render partial: "homepage/"+get_partial_path+"grid_item", collection: @listings, as: :listing, locals: { show_distance: location_in_use }
        elsif location_in_use
          render partial: "homepage/" + get_partial_path+"list_item_with_distance", collection: @listings, as: :listing, locals: { shape_name_map: shape_name_map, show_distance: location_in_use }
        else
          render partial: "homepage/" + get_partial_path+"list_item", collection: @listings, as: :listing, locals: { shape_name_map: shape_name_map }
        end
      }.on_error {
      ...
      map_search_result.on_success{ |listings|
      
        @listings_for_map = listings
      
      }
      if params[:selected_range].present?
          dt_range = params[:selected_range].split(" - ")
          st_dt = dt_range[0]
          en_dt = dt_range[1]
          seltd_range = (st_dt..en_dt).to_a
          @new_listing = []
          if search_result[:data].present?
            search_result[:data].each do |listing|
              listng = Listing.find(listing.id)
              left_range = seltd_range - listng.fetch_all_booked_and_block_dates
              if left_range.present?
                @new_listing.push listng
              end
            end
            distances = {}
            converted_listings = @new_listing.map do |listing|
              distance_hash = distances[listing.id] || {}
              ListingIndexService::Search::Converters.listing_hash(listing, includes, distance_hash)
            end
            new_search_result = Result::Success.new({count: @new_listing.count, listings: converted_listings}).and_then { |res| Result::Success.new(ListingIndexViewUtils.to_struct( result: res, includes: includes, page: current_page, per_page: per_page ) ) }
            new_search_result[:data].each do |qq|
              qq[:url] = "#{qq[:id]}-#{qq[:title].to_url}"
            end    
          else
        ...
        new_search_result.on_success { |listings|
        @listings = listings
        
        addRatingToListingsArray
        render action: get_partial_path+"index",locals: locals.merge(
  ...
  def addRatingToListingsArray
    @listings.each do |listing|

      @stars = 0
      transactions = Transaction.where(listing_id: listing.id)
      @totalVotes = transactions.joins(:testimonials).count
      @postiveVotes = transactions.joins(:testimonials).pluck(:grade).sum
    

      if @totalVotes > 0
        @stars = ( @postiveVotes / @totalVotes ) * 5
      end
