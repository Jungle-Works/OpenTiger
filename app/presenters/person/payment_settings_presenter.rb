class Person::PaymentSettingsPresenter
  ...
  def payments_enabled?
    paypal_enabled || stripe_enabled || razorpay_enabled
  end
  ...
  def show_verification_status  
    if razorpay_account_ready && razorpay_seller_account 
      @razorpay_seller_account["activation_details"]["status"]
    else  
      "Not Available"
    end 
  end

  def razorpay_seller_account 
    return @razorpay_seller_account if @razorpay_seller_account  
    @razorpay_seller_account = if razorpay_account_ready 
            razorpay_api.get_seller_account(account_id: razorpay_account[:razorpay_account_id], community: community.id)   
           end 
  end   

  def razorpay_account  
    @razorpay_account ||= razorpay_accounts_api.get(person_id: person.id, community_id: community.id).data || {}
  end   
  ...
  def razorpay_account_ready 
    @razorpay_account_ready ||= razorpay_account[:razorpay_account_id].present?
  end 


  def razorpay_account_verified?
    razorpay_account[:verification_status].present? ? @razorpay_account[:verification_status] == "activated" ? true : false : false 
  end  
  ...
  def razorpay_enabled
    @razorpay_enabled ||= RazorpayHelper.community_ready_for_payments?(community.id)
  end
