class ListingPresenter < MemoisticPresenter
  ...
  def razorpay_in_use
    RazorpayHelper.user_and_community_ready_for_payments?(@listing.author_id, @current_community.id)
  end
  ...
  def commission
    ...
    supported << :razorpay if razorpay_ready
    ...
    when matches([:paypal]), matches([:stripe]), matches([:razorpay]), matches([[:razorpay, :paypal]]), matches([[:razorpay, :stripe]]), matches([[:paypal, :stripe]]), matches([[:paypal, :stripe, :razorpay]])
      ...
      razorpay_settings = Maybe(payment_settings_api.get_active_by_gateway(community_id: @current_community.id, payment_gateway: :razorpay))
      .select {|res| res[:success]}
      .map {|res| res[:data]}
      .or_else({})
