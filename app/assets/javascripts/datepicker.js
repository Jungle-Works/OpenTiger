...
(function(module) {
  ...
  var setupPerDayOrNight = function(options, blockedDates,min_booking_days,user_id) {
    ...
    initializeFromToDatePicker('datepicker', {disabledDates: disabledDates, blockedDates: blockedDates, endDate: endDate, nightPicker: quantityNight, min_booking_days: min_booking_days },user_id);
  };
  ...
  */
  var initializeFromToDatePicker = function(rangeContainerId, opts,user_id) {
    opts = opts || {};
    console.log(opts)
    ...
    var blockedDates = opts.blockedDates || [];

    var blockedDatesArr = blockedDates.map(function(d) {
      var clonedDate = new Date(d);
      return clonedDate;
    });

    disabledEndDates = disabledEndDates.concat(blockedDatesArr);
    disabledStartDates = disabledStartDates.concat(blockedDatesArr);
    ...
    var options = {
      ...
      plusOne: nightPicker,
      gapInDates: (nightPicker || user_id == "181822")?  opts.min_booking_days : (opts.min_booking_days - 1)
    };
    ...
    picker.on('changeDate', function(e) {
      ...
      if (outputElementId === "booking-end-output" && !nightPicker && (user_id != "181822")) {
