...
var environment,stripeV3,adminStripeKey;
...
function initialize_defaults(locale,env,stripeKey) {
...
function initialize_user_feedback_form(locale,google_recaptcha_enabled) {
  ...
  $(form_id).validate({
    ...
    invalidHandler: function(form){
      if(google_recaptcha_enabled && !(grecaptcha && grecaptcha.getResponse().length)){
        googleRecaptchaCallback("");
      }
  ...
  return new Promise(function(resolve, reject){
      if (!window.Stripe) {
        ...
        script.onload = function(){
          stripeV3 = window.Stripe(adminStripeKey);
          resolve(true);
        };
