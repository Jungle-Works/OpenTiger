...
(function(module) {
  ...
  function restrictPriceInput(id){
    var oldValue = '';
        var newValue = '';
  ...
  module.initialize_new_listing_form = function(
    ...
    restrictPriceInput("#listing_price");
    ...
    numeric_field_names.push("listing[min_booking_days]");
    var numericRules = numeric_field_names.reduce(function(rules, name) {
      var el = module.utils.findElementByName(name);
      var min = el.data("min");
      var max = el.data("max");

      if(name == "listing[min_booking_days]"){
        rules[name] = {number_min: min, number_max: max, required: true};
      }
      else{
  ...
  module.moveToNextStep = function moveToNextStep(e,selectedLink,linksArray){
    e.preventDefault();
    
    if(!$(form_id).valid()){
      module.listingForm.focusInvalid();
      return;
    }
    currentSelectedLink = selectedLink+1;
    for(var i=0; i<linksArray.length; i++){
      if(i==selectedLink){
        $("#form-link-"+i).removeClass("selected-link");
        $("#form-panel-"+i).hide();
      }
      else if(i == selectedLink+1){
        $("#form-link-"+i).addClass("selected-link");
        $("#form-link-"+i).removeClass("disabled");
        $("#form-panel-"+i).show();
    ...
    setTimeout(function(){
      $([document.documentElement, document.body]).animate({
        scrollTop: $(".listing-form-panel").offset().top
    }, 300);
    },100);
  }

  module.selectForm = function selectForm(e,selectedLink,linksArray){
    e.preventDefault();
    if(currentSelectedLink!= selectedLink){
      $("#form-link-"+currentSelectedLink).removeClass("selected-link");
      $("#form-panel-"+currentSelectedLink).hide();
      $("#form-panel-"+selectedLink).show();
      $("#form-link-"+selectedLink).addClass("selected-link");
      $("#form-link-"+selectedLink).removeClass("disabled");
      currentSelectedLink = selectedLink;
    }
    
    
  }

  module.backToCategorySelection =function backToCategorySelection(){
    // window.history.back();
