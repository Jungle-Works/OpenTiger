...
class TransactionMailer < ActionMailer::Base
  ...
  def payment_receipt_to_seller(transaction, seller_model = nil, buyer_model = nil, community = nil)
    ...
    payment_total = payment[:payment_total].nil? ? payment[:total_price] : payment[:payment_total]
    if(community.id == 212)
    
      payment[:charged_commission] = Money.new(0.125 * payment_total , payment_total.currency)
  end
    ...
    gateway_fee =  Maybe(payment[:payment_gateway_fee]).or_else(Money.new(0, payment_total.currency))
  ...
  def payment_receipt_to_buyer(transaction, seller_model = nil, buyer_model = nil, community = nil)
    ...
    community ||= Community.find(transaction.community_id)
    payment = TransactionService::Transaction.payment_details(transaction)
    payment[:payment_total] = payment[:payment_total].nil? ? payment[:total_price] : payment[:payment_total]     
    if(community.id == 212)         
      tookanTaskEntry = TookanTask.find_by(transaction_id: transaction.id)
      if tookanTaskEntry.present?
        begin
          transaction.shipping_price = tookanTaskEntry[:task_delivery_charges]
          payment[:payment_total] = Money.new(payment[:payment_total] + transaction.shipping_price,   payment[:payment_total].currency)
        rescue => e  
          puts "ther was erooorrrrr @@@@@@@@" 
        end  
      end
    end
  ...
  def build_url_params(community, recipient, ref="email")
    {
      host: community.full_domain(with_protocol: true),
      ref: ref,
