...
class PersonMailer < ActionMailer::Base
  ...
  def email_confirmation(email, community)
    ...
    puts " Confirmation Email sent with host : " , @host
  ...
  def reset_password_for_user_by_admin(person, email_address, reset_token, community)
  ...
  def welcome_email(person, community, regular_email=nil, test_email=false)
    ...
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      ...
      @url_params = {}
      @url_params[:host] = "#{community.full_domain(with_protocol: true)}"
      ...
      puts " the recipient id is " , @recipient.id
      puts " the recipient is admin " , @recipient[:is_admin]
      puts " the recipient condition " , @recipient[:is_admin]==1 && !@test_email
      subject = if @recipient[:is_admin]==1 && !@test_email
