...
class Participation < ApplicationRecord
  ...
  scope :direct_participants, -> {
    joins(:conversation)
    .where( { conversations: { starting_page: "profile" }} )
  }
  scope :starter_without_direct_participants, -> {
    starter.joins(:conversation)
    .where( { conversations: { starting_page: [nil,"payment","listing"] }} )
  }
  scope :recipient_without_direct_participants, -> {
    recipient.left_joins(:conversation)
    .where( { conversations: { starting_page: [nil,"payment","listing"] }} )
  }
end
