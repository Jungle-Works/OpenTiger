...
class Listing < ApplicationRecord
  ...
  has_many :listing_blockdates

  attr_accessor :blocked_dates, :star
  ...
  after_destroy :notify_about_deleted_listing

  def fetch_all_booked_and_block_dates
    (listing_blockdates.map{|a| a.date.strftime("%Y-%m-%d")} + fetch_all_bookings_dates.map{|b| b.strftime("%Y-%m-%d")}).uniq
  end

  def app_fetch_all_booked_and_block_dates
    final_array = []
    if listing_blockdates.present?
      listing_blockdates.each do |listing_blockdate|
        blockdate_hsh = HashWithIndifferentAccess.new
        blockdate_hsh[:id] = ""
        blockdate_hsh[:date] = listing_blockdate.date
        final_array.push blockdate_hsh
      end
    end
    if fetch_all_bookings_dates.present?
      fetch_all_bookings_dates.each do |booked_date|
        bookeddate_hsh = HashWithIndifferentAccess.new
        bookeddate_hsh[:id] = ""
        bookeddate_hsh[:date] = booked_date
        final_array.push bookeddate_hsh
      end
    end
    return final_array
  end

  def fetch_all_bookings_dates
    return [] unless tx.present?
    booking_dates = []
    tx.each do |t|
      booking_dates += (t.booking.start_on..(t.booking.end_on - 1.day)).to_a if t.booking.present? &&
      t.booking.start_on.present? && t.booking.end_on.present? 
    end
    return booking_dates.uniq 
  end

  def has_booking_and_blocked_dates(start_date,end_date)
    return false unless start_date.present? && end_date.present?
    start_date = start_date.to_date
    end_date = end_date.to_date - 1.day
    return true if start_date < Date.today || end_date < Date.today || check_between_blocked_dates(start_date,end_date) || check_between_booking_dates(start_date,end_date)
    return false
  end

  def check_between_booking_dates(start_date,end_date)
    return true if tx.joins(:booking).where("(start_on >= ? and start_on <= ?) or (end_on > ? and end_on < ?) or (start_on <= ? and end_on > ?)", start_date, end_date, start_date, end_date, start_date, end_date).present?
    return false    
  end

  def check_between_blocked_dates(start_date,end_date)
    return true if listing_blockdates.where(:date => start_date..end_date).present?
    return false    
  end
  ...
  def self.delete_by_author(author_id)
   ...
   def self.listing_location(latitude, longitude)
    return Location.select("( 6371 * ACOS( COS( RADIANS( #{latitude} ) ) * COS( RADIANS(locations.`latitude` ) ) * COS( RADIANS(locations.`longitude` ) - RADIANS( #{longitude} ) ) + SIN( RADIANS( #{latitude}) ) * SIN( RADIANS(`latitude` ) ) ) ) AS distance")
  end

  def cal_total_positive_votes
    tx.joins(:testimonials).pluck(:grade).sum
  end

  def actionButton
    CommunityTranslation.find_by(community_id: community_id,translation_key: action_button_tr_key)[:translation]
  end
  ...
  begin
    ...
    puts " error occured while getting price per cent"
  ...
  def cal_total_votes
    tx.joins(:testimonials).count
  end

  def cal_star_rating
    return ((cal_total_positive_votes/cal_total_votes.to_f) * 5 ).round if cal_total_votes > 0
    return 0
  end

  def get_custom_field_name(id)

    CustomFieldName.find_by(custom_field_id: id)[:value]

  end

  def getProcess

    TransactionProcess.find(transaction_process_id).process

  end

  def selection_name(id)

    
  end

  def get_listing_review
    tx.joins(:testimonials)
  end

  def self.by_category_id(cat_ids)
    return where(nil) unless cat_ids.present?
    final_cat_ids = Category.final_category_ids_for_listing_filters(cat_ids)
    return where(:category_id => final_cat_ids)
  end

  def self.by_listing_shape(listing_shape_ids)
    return where(nil) unless listing_shape_ids.present?
    return where(:listing_shape_id => listing_shape_ids)
  end

  def self.by_price_range(min, max)
    return where(nil) unless min.present? && max.present?
    return where(:price_cents => min..max)
  end

  def self.by_title(text_srch)
    return where(nil) unless text_srch.present?
    return where("title like ?","%#{text_srch}%")
  end

  def self.by_order(order_by, order)
    return order("created_at DESC") unless order_by.present? && order.present?
    return left_joins(:comments).group(:id).order("COUNT(comments.id) #{order}") if order_by == "comment"
    return order("#{order_by} #{order}") if order_by == "title"
    return order("created_at DESC")
  end

   #Notify about delete listing
   def notify_about_deleted_listing
    hsh = HashWithIndifferentAccess.new
    hsh[:title] = author.displayed_name
    hsh[:description] = "Delete Listing"
    hsh[:notification_sender_id] = author.id
    hsh[:notification_sender_name] = author.displayed_name
    hsh[:notification_sender_image] = author.image_url
    hsh[:notification_type] = 2 #delete_listing
