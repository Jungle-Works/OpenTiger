...
class Category < ApplicationRecord
  ...
  def get_active_filter_detail
    ...
    if custom_fields.present?
      custom_fields.each do |cus_field|
  ...
  def self.final_category_ids_for_listing_filters(cat_ids)
    child_cats = where(id: cat_ids).where.not(parent_id: nil)
    fetch_child_parent_ids = child_cats.pluck(:parent_id).uniq
    parent_cat_ids = where(id: cat_ids).where(parent_id: nil).ids
    left_parent_category_with_no_child_in_cat_ids_group = parent_cat_ids - fetch_child_parent_ids
    final_cat_ids = child_cats.ids + parent_cat_ids
    where(id: left_parent_category_with_no_child_in_cat_ids_group).each do |pid|
      final_cat_ids += pid.subcategory_ids
    end
    return final_cat_ids
  end

  def active_listing_shapes
    listing_shapes.where(deleted: false)
  end
