...
class Person < ApplicationRecord  
  ...
  # For API <START>
    def create_token
      ...
      check = Person.find_by_app_password(random_token)
      if check.present?
        return create_token
      else
        update_attributes(:app_password => random_token)
      end
    end

    def get_person_complete_detail
      hsh = HashWithIndifferentAccess.new
      hsh[:message] = "Successful"
      hsh[:status] = "200"
      return hsh
    end
    def get_person_signup_info
      hsh = HashWithIndifferentAccess.new
      hsh[:community_id] = community_id
      hsh[:person_id] = id
      return hsh
    end
    ...
    def avg_rating
      self.received_testimonials.average(:grade).to_f.round
    end
    ...
    def displayed_name
      return PersonViewUtils.person_display_name(self,community)
    end
  ...
  def send_notification(hsh)
    ios_devices = tb_active_app_sessions.where("device_token is not null").where("device_token != ?","").where(device_type: "IOS")
    android_devices = tb_active_app_sessions.where("device_token is not null").where("device_token != ?","").where(device_type: "ANDROID")
    if ios_devices.present?
      p "Ios device found are:", ios_devices.ids
      notification_ios(hsh, ios_devices.pluck(:device_token))
    else
      p "No ios device found for person id:", id
    end
    if android_devices.present?
      p "Android device found are:", android_devices.ids
      notification_android(hsh, android_devices.pluck(:device_token))
    else
      p "No android device found for person id:", id
    end
  end

  def notification_android(hsh, device_tokens)
    require 'fcm'
    community_fcm_key = community.try(:android_fcm_key)
    if community_fcm_key.present?
      fcm = FCM.new(community_fcm_key)
      registration_ids= device_tokens # an array of one or more client registration tokens # an array of one or more client registration tokens
      options = { data: {wasTapped: true, title: hsh[:title], text_message: hsh[:description], :notification_sender_id => hsh[:notification_sender_id], :notification_type => hsh[:notification_type], :notification_type_id => hsh[:notification_type_id], :sender_name => hsh[:notification_sender_name], :sender_image => hsh[:notification_sender_image], :conversation_id => hsh[:conversation_id], :starting_page => hsh[:starting_page], :participation_id => hsh[:participation_id], :data => hsh[:data]}, priority: "high", notification: {}}
      response = fcm.send(registration_ids, options)
    else
      "Andorid fcm key not found."
    end
  end

  #Notification send ios
  def notification_ios(hsh, device_tokens)
    app=RailsPushNotifications::APNSApp.where(community_id: community_id).last
    if app.present?
      begin
        p "device token is:", device_tokens
        notification = app.notifications.create!(
          destinations: device_tokens,
          data: { aps: { alert: hsh[:description], sound: 'default', badge: 1 , notification_type: hsh[:notification_type],
          custom: { "content" => hsh },} }
          )
        res= app.push_notifications
      rescue => e  
        puts e
      end
    else
      p "Ios app not found for push."
    end
  end 
  ...
  def msg_hsh
    ...
    hsh[:given_name] = given_name
    hsh[:displayed_name] = displayed_name
    return hsh
  end
  ...
  # paypal account.
  def paypal_in_use(community_id)
    PaypalHelper.user_and_community_ready_for_payments?(id, community_id)
  end

  # Check stripe is activated or not
  def stripe_in_use(community_id)
    StripeHelper.user_and_community_ready_for_payments?(id, community_id)
  end

  def is_posting_allowed
    #If marketplace owner has enable option and has not allowed this user for post listing.
    if community.require_verification_to_post_listings.present? && !community_memberships.last.try(:can_post_listings).present?
      return false
    else
      return true
    end
  ...
  class << self
    def search_by_pattern_sql(table, pattern=':pattern')
      "(#{table}.given_name LIKE #{pattern} OR #{table}.family_name LIKE #{pattern} OR CONCAT_WS(' ', #{table}.given_name, #{table}.family_name) LIKE #{pattern} OR #{table}.display_name LIKE #{pattern})"
    end
  end
end
