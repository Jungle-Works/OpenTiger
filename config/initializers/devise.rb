...
module DatabaseAuthenticatableHelpers
  ...
  def find_by_username_or_email(login, community_id)
    # Person
    #   .joins("LEFT JOIN emails ON emails.person_id = people.id")
    ...
    #          .first
    puts " the find by username or email is checked ... "
    c = Person
    .joins("LEFT JOIN emails  ON emails.person_id = people.id")
    .where("(people.community_id = :cid) AND (people.username = :login OR emails.address = :login) ",
    cid: community_id, login: login)
    .first
...
Devise.setup do |config|
  ...
  config.mailer_sender = "Yelo <contact@yelo.red>"
  ...
  config.omniauth :google_oauth2, setup: true, :skip_jwt => true 
