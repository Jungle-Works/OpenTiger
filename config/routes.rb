...
Rails.application.routes.draw do
  ...
  get "/pages/content/:id" => "content_page_load#load"
  ...
  namespace :api do
    namespace :v1 do
      resources :customers , only: [], defaults: { format: :json } do
        collection do
          post :sign_up
          post :login
        end
      end
      get    "/update_password"          => "people#update_password"
      post   "/customer_signup"          => "customers#customer_signup"
      post   "/login_via_person_id"      => "customers#login_via_person_id"
      post   "/check_email"              => "customers#check_register_email"
      post   "/customer_login"           => "customers#customer_login"
      post   "/get_listing"              => "app_listings#get_all_listing"
      post   "/get_listing_details"      => "app_listings#get_listing_details"
      post   "/get_app_config"           => "customers#get_app_settings"
      post   "/delete_comment"           => "app_listings#delete_comment"
      post   "/checkout"                 => "app_listings#checkout"
      post   "/inbox"                    => "customers#getMessages"
      post   "/tookan_delivery_charge"   =>  "customers#getDeliveryCharge"
      post   "/messageTrail"             =>  "customers#getMessageTrail"
      post   "/sendMessage"              =>  "customers#sendMessage"
      post   "/msg_detail"              =>  "customers#msg_detail"
      post   "/orderDetails"             =>  "customers#orderDetails"
      post   "/customer_logout"          => "customers#logout"
      post   "/customer_change_password" => "customers#change_password"
      post   "/customer_forgot_password" =>  "customers#forgot_password"
      post   "/customer_update_profile"  => "customers#update_profile"
      post   "/get_active_filters"       => "app_listings#get_active_filters"
      post   "/my_listings"              => "app_listings#my_listings"
      post   "/listing_image_config"     => "app_listings#image_config"
      post   "/post_listing"             => "app_listings#add_listing"
      post   "/add_person_image"         => "app_listings#addPersonImage"
      post   "/generate_token"           => "customers#generate_token"
      get    "/app_payment_callback"     => "customers#app_payment_callback"
      post   "/mark_as_read"             => "app_listings#message_read"
      post   "/accept_reject_payment"    => "customers#accept_reject_payment"
      post   "/mark_as_complete_dispute" => "customers#mark_as_complete_dispute"
      post   "/feedback"                 => "customers#feedback"
      post   "/fb_login"                 => "customers#fb_login"
      post   "/check_policies"           => "customers#check_policies"
    end
  end
  ...
  get "/ui_api/hippo_props" => "topbar_api#hippo_props"
  ...
  resources :communities, only: [:new, :create, :secondStep, :third_step]
  ...
  scope "(/:locale)", :constraints => { :locale => locale_matcher } do
    ...
    get "/login_v2" => "sessions#login_via_access_token"
    ...
    post '/payment_settings/razorpay_account' => 'payment_settings#add_razorpay_account', :as => :add_razorpay_account
    ...
    namespace :razorpay_service do  
      resources :checkout do   
        collection do  
          ...
          get :payout
          post :order_create   
          get "/redirect/:id" => "checkout#redirect",   as: :redirect
          post :verify
        end 
      end 
    end  
    ...
    namespace :admin do
      ...
      resources :content_pages do
        collection do
          get :add_content_page
          post :new_content_page
        end
        member do
          get :delete_content_page
          post :update_content_page
      ...
      get "/postings" => "communities#postings"

      # get "/domain" => "admin_domains#show"

      get "/approveListing/:id" => "communities#approveListing"

      get "/dissaproveListing/:id" => "communities#dissaproveListing"
      ...
      get "getting_started_guide/theme"                  => "getting_started_guide#theme",                  as: :getting_started_guide_theme
      get "getting_started_guide/skip_payment"           => "getting_started_guide#skip_payment",           as: :getting_started_guide_skip_payment
      get "getting_started_guide/skip_theme"             => "getting_started_guide#skip_theme",             as: :getting_started_guide_skip_theme
      ...
      resources :checkout_settings do    
        collection do  
          get :add_checkout_field
          get :change_checkout_type
          get :update_checkout_field
        end
        member do
          get :delete_checkout_field
        end
      end  

      
      resources :manage_landing_page do
        collection do
          post :preview
          get :enable_lp
          get :disable_lp
          get :enable_disable_lp
          post :save_only
          get :save_and_release
        end
      end
    ...
    resources :listings do
      member do
        ...
        delete :unfollow
        delete :delete
        get :listing_delete , to: 'listings#delete'
      end
      collection do
