...
class SearchBar extends Component {
  ...
  render() {
    const { mode, keywordPlaceholder, locationPlaceholder, keywordQuery, locationQuery,flexTheme } = this.props;
    ...
    const locationInput = input({
      ...
      placeholder: locationPlaceholder||"Location...",
    ...
    return div({
      ...
      classSet: {
        ...
        [css.rootNewTheme]: flexTheme || false
      },
      // style: (true ? {width:"100%","borderLeft": "1px solid #e7e7e7"}: {})
    }, [
      button({
      ...
      form({
        classSet: { [css.form]: true ,[css.newThemeForm]: flexTheme || false },
        onSubmit: (e) => {
          e.preventDefault();
          this.handleSubmit();
        },
        ...
        button({
          ...
          classSet: {[css.searchButtonNewTheme]: flexTheme||false},
          dangerouslySetInnerHTML: { __html: icon },
          style: {
            backgroundColor: this.state.mobileSearchOpen ? bgColorDarkened : 'transparent',
          },
        }),
        span({ className: css.focusContainer,style: flexTheme? {display: "none"}: {} }),
      ]),
    ]);
  }
}
