...
export default function AddNewListingButton({ text, url, customColor, className, mobileLayoutOnly,flexTheme }) {
  ...
  return flexTheme ? newTheme(text,color,hoverColor,className,url,mobileLayoutOnly):
  a({
...
function newTheme(text,color,hoverColor,className,url,mobileLayoutOnly){
  const buttonText = `+ ${text}`;
  return a({
    className: classNames(className, 'AddNewThemeListingButton', css.themeButton, { [css.responsiveLayout]: !mobileLayoutOnly }),
    href: url,
    title: text
  }, [
    ...
    span({
      className: 'AddNewListingButton_desktop',
      classSet: { [css.desktopNewTheme]: true },
      style: { color }

    }, buttonText),
    span({className: classNames("logo-border", css.logoBorder)},"")
  ]);
}
